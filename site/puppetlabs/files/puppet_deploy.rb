  ...
  def make_subbranch( branch_to_make , make_as=nil )
    ...
    if File.directory? branch_dir
      ...
      Dir.chdir branch_dir do
        ...
        pp_and_system "LANG='C' git submodule #{$gitnoise} update --init" unless %x{ git submodule status }.empty?
      end
    else
      ...
      Dir.chdir @env_base_dir do
        ...
        pp_and_system "LANG='C' cd #{checkout_as} && git submodule #{$gitnoise} update --init" unless %x< cd #{checkout_as} && git submodule status >.empty?
      end
    end
  end
