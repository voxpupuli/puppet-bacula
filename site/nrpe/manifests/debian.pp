...
class nrpe::debian {
	...
	file { "nrpe.commands.cfg":
		...
		group      => 'nagios',
		mode       => '0400',
		source     => "puppet:///nrpe//nrpe.commands.i386.cfg",
		require    => [ Package["nagios-nrpe-server"], Class["sudo"] ],
		notify     => Service["nrpe"],
	}
	...
	service { "nrpe":
		...
		require    => [ Package["nagios-nrpe-server"], File["nrpe.cfg"], File["nrpe.commands.cfg"] ],
	}
}
