class service::bootserver {
  ...
  $ubuntu_common = {
    ...
    append  => "vga=791 initrd=images/<%= os %>/<%= ver %>/<%= arch %>/initrd.gz auto locale=en_US console-setup/layoutcode=us netcfg/get_hostname=debian url=http://ran.dc1.puppetlabs.net/d-i/ubuntu_base.cfg text",
  }
  ...
  $debian_common = {
    "file"    => "os_<%= os %>",
    "kernel"  => "images/<%= os %>/<%= ver %>/<%= arch %>/linux",
    "append"  => "vga=normal initrd=images/<%= os %>/<%= ver %>/<%= arch %>/initrd.gz auto locale=en_US console-keymaps-at/keymap=us hostname=<%= os %> domain=unknown url=http://ran.dc1.puppetlabs.net/d-i/debian_base.cfg text",
  }
